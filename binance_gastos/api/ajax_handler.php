    ...
    switch ($data->action) {
        ...
        case 'getUserExpenses':
            ...
            if (isset($_SESSION['user_id'])) {
                ...
                $groupId = $data->groupId;
                $gastoController = new GastoController($db);
                $response = $gastoController->getUserExpenses($userId, $groupId);
            } else {
                $response = ["success" => false, "message" => "Usuario no autenticado"];
            }
            break;
        ...
        case 'getTransactions':
            ...
            if (isset($_SESSION['user_id'])) {
                $gastoController = new GastoController($db);
                $response = $gastoController->getTransactions($data->groupId);
            } else {
                $response = ["success" => false, "message" => "Usuario no autenticado"];
            }
            break;
        case 'calculateMinTransactions':
            session_start();
            if (isset($_SESSION['user_id'])) {
                $userId = $_SESSION['user_id'];
                $groupId = $data->groupId;
                $gastoController = new GastoController($db);
                $response = $gastoController->calculateMinTransactions($userId, $groupId);
            } else {
                $response = ["success" => false, "message" => "Usuario no autenticado"];
            }
            break;
        case 'markAsPaid':
            ...
            if (isset($_SESSION['user_id'])) {
                ...
                $acreedorId = $data->acreedor_id;
                $cantidad = $data->cantidad;
                $groupId = $data->groupId; // Asegúrate de que groupId esté definido
                $gastoController = new GastoController($db);
                $response = $gastoController->markAsPaid($deudorId, $acreedorId, $cantidad, $groupId);
            } else {
                $response = ["success" => false, "message" => "Usuario no autenticado"];
            }
            break;
